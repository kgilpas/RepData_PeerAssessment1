...
library(ggplot2)
ggplot(data=df ,aes(x=s))+geom_histogram( stat="bin") + xlab("Total Steps per Day")+ylab("Frequency")+ggtitle("Histogram of Total Steps per Day")
```
...
plot(x=df2$i, y=df2$avgSteps,type="l", xlab="interval (5 Minute)",ylab="Number of Steps (Avg.)", main="Time Series Plot")
```
...
new$time <- strptime(paste(new$date,sapply(new$interval, formatC, width = 4, flag = 0)), format = "%Y-%m-%d %H%M")

# Create a vector called impute which is needed for the subsequent for loop to impute the variable 'steps'
impute<-rep(round(avgSteps),61)
# Loop to impute the NA with the average steps by interval
for (i in 1:nrow(new)){ if (is.na(new[i,1])) { new[i,1]=impute[i]} else {next}  }
...
ggplot(data=df.new ,aes(x=s.new))+geom_histogram( stat="bin") + xlab("Total Steps per Day")+ylab("Frequency")+ggtitle("Histogram of Total Steps per Day (Imputed)")
...
```{r}
new$wkdaytyp <- ifelse(weekdays(new$time)=="Saturday" | weekdays(new$time)=="Sunday","weekend","weekday"    )

weekend <- subset(new,new$wkdaytyp=="weekend")
we.avgSteps <- round(as.vector(tapply(weekend$steps,weekend$interval,mean,na.rm=T)),1)
...
wd.avgSteps <- round(as.vector(tapply(weekday$steps,weekday$interval,mean,na.rm=T)),1)
